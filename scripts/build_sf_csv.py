# build_sf_rag_csv.py
"""
Create a RAG‑ready CSV for Where2Live from raw datasets
-----------------------------------------------------------
1. listings.csv (InsideAirbnb – San Francisco)
...
try:
    ...
    # Try to identify the ZIP code column
...
if isinstance(zip_gdf, gpd.GeoDataFrame) and 'geometry' in zip_gdf.columns:
    ...
    # Fallback to non-spatial join if geometry isn't available
...
# Load and join school district data
print("Loading school district data …")
try:
    dist_df = pd.read_csv(DIST_DATA)
    # Clean up the district data - some columns might have leading/trailing whitespace
    dist_df = dist_df.apply(lambda x: x.str.strip() if isinstance(x, pd.Series) and x.dtype == "object" else x)
    
    # We'll use a simpler approach - just keep the district name and location data
    dist_df = dist_df[["NAME", "INTPTLAT", "INTPTLON"]].copy()
    
    # Convert lat/lon to numeric, dropping any invalid values
    ...
    # Create a GeoDataFrame with the centroid points
...
# Compute nearby‑school poverty estimate (BallTree haversine)
...
# Load and process parks/recreation facilities data
...
try:
    ...
    # Create the output DataFrame with proper column selection and renaming
    column_mapping = {
        'id_left': 'listing_id',
...
nearby_facilities number of parks/rec facilities within 1km (integer)
